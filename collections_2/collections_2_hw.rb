...
first_letter_r("ruby", "rspec" "rails")
...
#Create a new array from the captain_planet array with all the elements that contain the letter "a".  
  ...
  def new_capt_planet array
    new_planet_array = []
    array.each do |element|
      if element.include?('a')
        new_planet_array << element
      end
      new_planet_array
    end
...
Identify the first element in the stuff array that begins with the letters "wa".
  ...
  def first_letters_wa array
    new_array =[]
    array.each do |word|
      if word[0] == "w" and word[1] != "a"
        break
      else
        new_array << word
      end
...
#stuff = ["candy", :pepper, "wall", :ball, "wacky"]
  def all_first_letters_wa array
        array.select { |word| word[0..1] == "wa" }
      end
    end
